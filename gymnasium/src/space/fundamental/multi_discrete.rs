...
use dfdx::{
    ...
    tensor::{AsArray, HasErr, SampleTensor, Storage, Tensor, TensorFromVec, TensorToArray},
};
    ...
    D: Storage<E> + TensorFromVec<E>,
{
    ...
    pub fn n(&self) -> &Tensor<S, E, D> {
        &self.n
    }

    pub fn start(&self) -> &Tensor<S, E, D> {
        &self.start
    }

    pub fn end(&self) -> &Tensor<S, E, D> {
        &self.end
    }
...
where
    S: Shape,
    E: Dtype + PrimInt + SampleUniform,
    D: Storage<E> + SampleTensor<E> + TensorFromVec<E>,
{
    type Shape = S;
    type Dtype = E;
    type Storage = D;
...
where
    S: Shape,
    E: Dtype + PrimInt + SampleUniform,
    D: Storage<E> + TensorFromVec<E> + TensorToArray<S, E>,
{
