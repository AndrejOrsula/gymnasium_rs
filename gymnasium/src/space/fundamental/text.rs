...
use dfdx::{
    ...
    tensor::{HasErr, SampleTensor, Storage, Tensor, TensorFromVec},
};
use rand::{
...
where
    D: Storage<u8> + SampleTensor<u8> + TensorFromVec<u8>,
{
    rng: Arc<Mutex<Rng>>,
    ...
    D: Storage<u8> + SampleTensor<u8> + TensorFromVec<u8>,
{
    ...
    pub fn seed(&mut self, seed: Option<u64>) {
        *self.rng.lock().unwrap() = Self::new_rng(seed);
    }
    ...
    fn check_space(min_len: usize, max_len: usize) -> Result<()> {
        ...
        if min_len > max_len {
            return Err(GymnasiumError::InvalidSpace(format!(
                "The minimum length of the text space cannot be greater than the maximum length (min_len: {}, max_len: {})",
                min_len, max_len
            )));
        }
    ...
    D: Storage<u8> + SampleTensor<u8> + TensorFromVec<u8>,
{
    ...
    fn contains(&self, value: &Tensor<Self::Shape, Self::Dtype, Self::Storage>) -> bool {
        if let Ok(value) = String::from_utf8(value.as_vec()) {
            self.contains_str(&value)
        } else {
            false
        }
    }
